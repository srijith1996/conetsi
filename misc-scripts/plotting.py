...
def plot_violin(X, title="", xlabel="", ylabel="", scale='auto',
  ...
  def autolabel():
    ...
    for i in range(0,len(pos)):
      ...
      ax.text(pos[i]+0.1, means[i],
              '%.3g' % means[i],
              ha='center', va=va_mean, color='darkred', fontsize=fs_norm-3)
  ...
  #plt.tight_layout()
  plt.subplots_adjust(bottom=0.20, right=0.97, top=0.97, left=0.13)
  ...
  if same_subgroups and len(groups) > 1:
    ...
    for i, lobe in enumerate(res['bodies']):
      lobe.set(color=colorWheel[(6 % groups[0]) + 5], alpha=0.55)
  ...
  if leg == True:
    ax.legend(handles=custom_lines, labels=['mean', 'median', 'max'], shadow=False,
              fontsize=fs_norm-4,ncol=1,loc=(0.01, 0.65), framealpha=0.50)
